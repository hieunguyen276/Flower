...
export const createItem = Joi.object({
    name: Joi.string().trim().max(MAX_STRING_SIZE).required().label("Tên bề mặt")
        .custom(
            (value, helpers) =>
                new AsyncValidate(value, async function () {
                    const surface = await Surface.findOne({name: value});
                    return !surface ? value : helpers.error("any.exists");
                }),
        ),
